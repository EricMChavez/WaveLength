...
describe('meter-slice', () => {
  it('defaults all 6 meters to hidden mode', () => {
  ...
  it('initializeMeters sets active slots to their direction as mode', () => {
    ...
    expect(get().meterSlots.get(meterKey(2))!.mode).toBe('hidden');
    expect(get().meterSlots.get(meterKey(3))!.mode).toBe('output');
    expect(get().meterSlots.get(meterKey(4))!.mode).toBe('hidden');
  });
  ...
  it('setMeterMode updates a single slot', () => {
    ...
    expect(get().meterSlots.get(meterKey(1))!.mode).toBe('hidden');
  });
